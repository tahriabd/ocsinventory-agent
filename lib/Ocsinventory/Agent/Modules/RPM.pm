...
sub run {
  ...
  my @list;
  my $buff;
  foreach (`rpm -qa --queryformat "%{NAME}.%{ARCH} %{VERSION}-%{RELEASE} --%{SOURCERPM}-- --%{INSTALLTIME}-- --%{SIZE}-- %{SUMMARY}\n--\n" 2>/dev/null`) {
    if (! /^--/) {
      chomp;
      $buff .= $_;
    } elsif ($buff =~ s/^(\S+)\s+(\S+)\s+--(.*)--\s+--(.*)--\s+--(.*)--\s+(.*)//) {
    my ($name,$version,$packname,$installdate,$filesize,$comments) = ( $1,$2,$3,$4,$5,$6 );
    my @words = split("-".$version.".src.rpm",$packname);
